...
public interface IQueueService
{
    ...
    Task<ApiResponse<int>> CloseAllSessionsForDateAsync(Guid tenantId, DateTime date, Guid callerUserId);
    Task<ApiResponse<PagedResult<QueueSessionDto>>> GetSessionsAsync(Guid tenantId, Guid callerUserId, int pageNumber = 1, int pageSize = 10);
    Task<ApiResponse<QueueSessionDto>> GetSessionByIdAsync(Guid tenantId, Guid sessionId, Guid callerUserId);

    // Tickets
    Task<ApiResponse<QueueTicketDto>> IssueTicketAsync(Guid tenantId, CreateQueueTicketRequest request, Guid callerUserId);
    Task<ApiResponse<QueueTicketDto>> IssueTicketWithPaymentAsync(Guid tenantId, CreateQueueTicketWithPaymentRequest request, Guid callerUserId);
    ...
    // Views
    Task<ApiResponse<QueueBoardDto>> GetBoardAsync(Guid tenantId, Guid callerUserId);
