...
export default async function PartnerLoginPage({
  ...
  if (clinic?.tenantType && clinic.tenantType !== 'Partner') {
    ...
    <div className='min-h-screen w-full grid grid-cols-1 lg:grid-cols-2'>
      <div className='flex flex-col justify-center p-6 sm:p-12 bg-background relative'>
        <div className='w-full max-w-100 mx-auto'>
          <LoginForm
            ...
            portalType='Partner'
            heading='تسجيل دخول الشركاء'
            description='هذه البوابة مخصصة للشركاء وإدارة علاقات التعاون مع العيادات.'
            backHref={`/${tenantSlug}`}
            backLabel='العودة للصفحة العامة'
            submitLabel='دخول الشريك'
          />
        </div>
      </div>
      ...
      <div className='hidden lg:flex flex-col justify-between p-12 bg-muted/30 border-r border-border/50 relative overflow-hidden'>
        ...
        <div className='relative z-10 flex items-center gap-3'>
          ...
          <span className='font-bold text-xl tracking-tight'>{clinic?.clinicName || 'بوابة الشركاء'}</span>
        </div>

        <div className='relative z-10 space-y-4 max-w-sm'>
          <h1 className='text-4xl font-black tracking-tight text-foreground'>
            منصة الشركاء المتكاملة.
          </h1>
          <p className='text-lg font-medium text-muted-foreground'>
            دخول موحد لإدارة التعاقدات، التكامل مع العيادات، ومتابعة طلبات الشراكة.
          </p>
        </div>
      </div>
    </div>
